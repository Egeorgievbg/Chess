...
- Реалтайм социална логика: `app/socket_events.py` изпраща `friend_statuses` при връзка, а `app/static/js/social.js` обработва `friend_statuses` + `user_online/offline`, обновява статусите на картите и позволява/забранява „Онлайн игра“ бутоните с кратък статус текст.
...
## Interactive Lesson 01 – „Начало“ (Фигурите и техните сили)

Целта е да се даде ясен и интерактивен старт за децата чрез визуални „карти“, аудио подсказки и бързи упражнения:

1. **Фигури, имена и сила** – всяка фигура е представена с кратко описание, точка (пешка = 1, кон = 3 и т.н.) и анимиран харт во view с `AudioManager`, който проговаря „Конът скочи“ или „Ладията се движи по прави линии“ при hover/пип.
2. **Начална позиция като drag&drop пъзел** – интерактивен екран позволява да се плъзне всяка фигура на стартовия квадрат, при грешка се получава топчест feedback, при успех браузърът пуска позитивен звук, а системата отбелязва кадъра като „запомнен“.
3. **Трикове за запомняне + бутон „Започни“** – панел с лесни правила (например „Дамата обикаля свободно“, „Пешката става ферз“), и бутон `Започни`, който води към адаптирана игра от `/game/new`, където детето практикува прости ходове, на които вече е научено.

Този урок използва същите шаблони (`base.html`, `game/play.html`) и JS (първоначален `gameConfig`), така че бъдещите разширения могат да зареждат JSON с нови „карти“ за фигури, да включват `NotificationManager` за подсказки и да активират специален sound cue от `AudioManager`.
